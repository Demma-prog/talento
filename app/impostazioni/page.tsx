...
import { Bot, CalendarClock, Database, Save, ShieldCheck } from "lucide-react";
...
export default function SettingsPage(){return <AppShell title="Impostazioni" active="Impostazioni">
  <section className="settings-grid"><div className="settings-main">
    <GmailConnectionCard />
    <article className="panel setting-card"><div className="setting-title"><span><Bot/></span><div><h3>Estrazione Gemini</h3><p>Modello utilizzato per organizzare i dati del CV</p></div></div><label>Modello<select defaultValue="gemini-flash"><option value="gemini-flash">Gemini Flash</option><option value="rules">Solo estrazione tradizionale</option></select></label><label className="toggle-row"><span><b>Genera sintesi del profilo</b><small>Crea la card Bio quando non è presente nel CV</small></span><input type="checkbox" defaultChecked/></label></article>
  ...
  </div><aside className="settings-side"><article className="panel"><ShieldCheck className="settings-shield"/><h3>Sicurezza</h3><p>Credenziali Gmail, Gemini e Supabase saranno conservate esclusivamente come variabili protette sul backend.</p></article><article className="panel"><Database className="settings-shield"/><h3>Supabase</h3><p>Database non ancora collegato.</p><button className="primary-button purple-button"><Save size={15}/> Salva configurazione</button></article></aside></section>
</AppShell>}
