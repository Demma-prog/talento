...
type Totals={imported:number;updated:number;duplicate:number;failed:number;queued:number};
const emptyTotals:Totals={imported:0,updated:0,duplicate:0,failed:0,queued:0};
...
export default function ImportPage(){
    ...
    try{
      ...
      do{
        ...
        for(let offset=0;offset<scan.messages.length;offset+=3){
          ...
          if(result.errors?.length)setImportErrors(current=>[...current,...result.errors]);
          session={imported:session.imported+result.imported,updated:session.updated+result.updated,duplicate:session.duplicate+result.duplicate,failed:session.failed+result.failed,queued:session.queued+(result.queued??0)};
  ...
  return <AppShell title="Importa curriculum" active="Importa CV">
    ...
    <section className="import-grid"><article className="panel import-process"><div className="panel-head"><div><p className="eyebrow">Processo automatico</p><h3>Cosa sta facendo Talento</h3></div></div>{[[Mail,"Scorre tutto Gmail","Carica automaticamente una pagina dopo l’altra."],[FileSearch,"Seleziona il curriculum","Distingue il CV dagli allegati accessori."],[Sparkles,"Analizza con Qwen","Estrae localmente dati, percorso e competenze."],[CheckCircle2,"Aggiorna senza duplicati","Conserva un profilo per persona e il CV più recente."]].map(([Icon,title,copy],i)=>{const I=Icon as typeof Mail;return <div className="process-step" key={String(title)}><span>{i+1}</span><i><I size={20}/></i><div><strong>{String(title)}</strong><p>{String(copy)}</p></div></div>})}</article>
      <aside><article className="panel sync-card"><p className="eyebrow">Risultati sessione</p><h3>Riepilogo</h3><div className="sync-stat"><span>Nuovi candidati</span><b>{totals.imported}</b></div><div className="sync-stat"><span>Profili aggiornati</span><b>{totals.updated}</b></div><div className="sync-stat"><span>Già presenti</span><b>{totals.duplicate}</b></div><div className="sync-stat"><span>Da elaborare</span><b>{totals.queued}</b></div><div className="sync-stat"><span>Errori di importazione</span><b>{totals.failed}</b></div>{!running&&overallProcessed>0&&<button className="secondary-button full" onClick={start}><RotateCcw size={15}/> Esegui nuovamente</button>}</article><article className="privacy-note"><ShieldCheck/><div><strong>Tieni aperta questa pagina</strong><p>Nel piano gratuito il browser coordina il lavoro. Puoi usare altre schede, ma non chiudere questa pagina finché l’importazione non è terminata.</p></div></article></aside></section>
  </AppShell>
}
