...
const profiles=[['sales','Commessa/o'],['warehouse','Magazziniere'],['cashier','Cassiera/e'],['office','Ufficio'],['other','Altro']];
const sectors=[['retail','Retail e vendite'],['logistics','Logistica'],['administration','Amministrazione'],['hospitality','Ristorazione'],['other','Altro']];
...
export default async function CandidatesPage({searchParams}:{searchParams:Promise<CandidateFilters>}){
  ...
  return <AppShell title="Candidati" active="Candidati">
    ...
    <form className="panel candidate-filters" method="get"><div className="filter-search"><Search size={17}/><input name="q" defaultValue={filters.q} placeholder="Nome, email, ruolo o competenza..."/></div><div className="filter-grid">
      ...
      <label>Profilo<select name="profile" defaultValue={filters.profile??""}><option value="">Tutti</option>{profiles.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label>
      <label>Settore<select name="sector" defaultValue={filters.sector??""}><option value="">Tutti</option>{sectors.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label>
    ...
    <section className="panel candidates-table-card"><div className="people-table"><div className="people-row people-head"><span>Candidato</span><span>Profilo</span><span>Località</span><span>Stato</span><span>Ultimo CV</span></div>{people.length?people.map((p,i)=><Link href={demo?"/candidati/martina-russo":`/candidati/${p.id}`} className="people-row" key={p.id}><span className={`candidate-cell avatar-${i%5}`}><CandidatePhoto candidateId={p.id} initials={p.initials}/><span><b>{p.name}</b><small>{p.birthYear?`${new Date().getFullYear()-p.birthYear} anni`:"Età non indicata"} · {genderLabels[p.gender??""]??"Sesso non indicato"}</small></span></span><span className="profile-cell"><b>{p.role}</b><small>{p.experienceYears} anni di esperienza rilevata</small></span><span>{p.city}</span><span><em className={`status status-${p.status.toLowerCase().replaceAll(" ","-")}`}>{p.status}</em></span><span>{p.receivedAt}</span></Link>):<div className="filter-empty"><Search/><strong>Nessun candidato corrisponde ai filtri</strong><Link href="/candidati" className="text-link">Azzera la ricerca</Link></div>}</div></section>
  </AppShell>
}
