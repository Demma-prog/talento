...
const profiles=[['accounting','Contabilità'],['logistics','Logistica'],['marketing','Marketing'],['cashier','Cassiere/a'],['sales','Commesso/a'],['warehouse','Magazzino'],['office','Ufficio'],['other','Altro']];
...
export default async function CandidatesPage({searchParams}:{searchParams:Promise<CandidateFilters>}){
  ...
  return <AppShell title="Candidati" active="Candidati">
    ...
    <form className="panel candidate-filters" method="get"><div className="filter-search"><Search size={17}/><input name="q" defaultValue={filters.q} placeholder="Nome, email, ruolo o competenza..."/></div><div className="filter-grid">
      ...
      <label className="review-filter"><input type="checkbox" name="review" value="true" defaultChecked={filters.review==="true"}/><span>Solo da verificare</span></label><label className="review-filter"><input type="checkbox" name="protected" value="true" defaultChecked={filters.protected==="true"}/><span>Categoria protetta</span></label><button className="primary-button purple-button filter-submit"><Filter size={16}/> Applica filtri</button>
    </div></form>
