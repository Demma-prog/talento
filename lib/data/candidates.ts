...
export type CandidateFilters={q?:string;age_min?:string;age_max?:string;gender?:string;city?:string;profile?:string;status?:string;experience_min?:string;review?:string;protected?:string};
export type CandidateListItem={id:string;initials:string;name:string;role:string;city:string;status:string;receivedAt:string;birthYear:number|null;gender:string|null;experienceYears:number;profile:string;needsReview:boolean;protectedCategory:boolean};
...
function classifyRole(text:string){const value=normalize(text);if(/cassier|gestione cassa|operatore di cassa/.test(value))return"cashier";if(/commess|addett.{0,10}(alla |alle )?vendit|sales assistant|shop assistant|retail assistant/.test(value))return"sales";if(/magazzinier|warehouse worker|scaffalist|picking|carrellist|addett.{0,10}magazzin/.test(value))return"warehouse";if(/contabil|ragionier|fatturaz|prima nota|bilancio|amministrativ.{0,12}contab/.test(value))return"accounting";if(/responsabile logist|impiegat.{0,10}logistic|supply chain|spedizionier|coordinator.{0,10}trasport/.test(value))return"logistics";if(/marketing manager|digital marketing|social media manager|advertis|brand manager|copywrit/.test(value))return"marketing";if(/segretari|reception|back office|front office|impiegat.{0,12}amministrativ|data entry/.test(value))return"office";return"other"}
function classifyProfile(experiences:any[]){for(const item of experiences.slice(0,2)){const category=classifyRole(`${item.role??""} ${item.description??""}`);if(category!=="other")return category}return"other"}
...
export async function getCandidates(filters:CandidateFilters={}):Promise<{data:CandidateListItem[];demo:boolean;cities:string[];total:number}>{
  ...
  const {data,error}=await supabase.from("candidates").select("id,first_name,last_name,birth_year,declared_gender,email,city,status,needs_review,job_category,protected_category,latest_cv_received_at,experiences(role,company,description,start_date,end_date,is_current,sort_order),skills(name)").order("latest_cv_received_at",{ascending:false}).limit(1000);
  ...
  let people=(data??[]).map(row=>{const experiences=Array.isArray(row.experiences)?[...row.experiences].sort((a,b)=>a.sort_order-b.sort_order):[];const skillText=(row.skills??[]).map(skill=>skill.name).join(" ");const combined=`${experiences.map(item=>`${item.role??""} ${item.company??""} ${item.description??""}`).join(" ")} ${skillText}`;const profile=row.job_category??classifyProfile(experiences);return{id:row.id,initials:`${row.first_name?.[0]??""}${row.last_name?.[0]??""}`.toUpperCase(),name:`${row.first_name} ${row.last_name}`,role:experiences[0]?.role??"Ruolo non indicato",city:row.city??"—",status:statusLabels[row.status]??row.status,receivedAt:row.latest_cv_received_at?new Intl.DateTimeFormat("it-IT",{day:"2-digit",month:"short",year:"numeric"}).format(new Date(row.latest_cv_received_at)):"—",birthYear:row.birth_year,gender:row.declared_gender,experienceYears:experienceYears(experiences),profile,needsReview:row.needs_review,protectedCategory:Boolean(row.protected_category),search:normalize(`${row.first_name} ${row.last_name} ${row.email??""} ${row.city??""} ${combined}`)}});
  ...
  if(filters.protected==="true")people=people.filter(item=>item.protectedCategory);
