...
export function CandidateEditor({ candidateId, candidate, startOpen=false }: { candidateId:string; candidate:EditableCandidate; startOpen?:boolean }) {
  ...
  async function submit(event:FormEvent){event.preventDefault();setSaving(true);setError("");try{const {data}=await createClient().auth.getSession();if(!data.session)throw new Error("Sessione scaduta");const response=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/${candidateId}`,{method:"PATCH",headers:{Authorization:`Bearer ${data.session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({...form,birth_year:form.birth_year?Number(form.birth_year):null,declared_gender:form.declared_gender||null,mark_verified:true})});if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.detail??"Modifica non salvata");}setOpen(false);router.refresh();}catch(cause){setError(cause instanceof Error?cause.message:"Modifica non salvata");}finally{setSaving(false)}}
...
export function DeleteCandidateButton({candidateId,name}:{candidateId:string;name:string}){const router=useRouter();const [loading,setLoading]=useState(false);async function remove(){if(!window.confirm(`Eliminare definitivamente la scheda di ${name}? L'email originale resterà in Gmail.`))return;setLoading(true);const {data}=await createClient().auth.getSession();if(!data.session){setLoading(false);return}const response=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/${candidateId}`,{method:"DELETE",headers:{Authorization:`Bearer ${data.session.access_token}`}});if(response.ok)router.push("/candidati");else setLoading(false)}return <button className="danger-button" onClick={remove} disabled={loading}>{loading?<LoaderCircle className="spin" size={15}/>:<Trash2 size={15}/>} Elimina candidato</button>}

export function RetryPhotoButton({candidateId}:{candidateId:string}){const router=useRouter();const [loading,setLoading]=useState(false),[message,setMessage]=useState("");async function retry(){setLoading(true);setMessage("");const {data}=await createClient().auth.getSession();const response=await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/${candidateId}/photo/retry`,{method:"POST",headers:{Authorization:`Bearer ${data.session?.access_token??""}`}});const body=await response.json().catch(()=>({}));setLoading(false);if(response.ok){setMessage("Foto aggiornata");router.refresh()}else setMessage(body.detail??"Foto non recuperata")}return <div className="photo-retry"><button className="secondary-button" onClick={retry} disabled={loading}>{loading?<LoaderCircle className="spin" size={15}/>:<Camera size={15}/>} Recupera foto</button>{message&&<small>{message}</small>}</div>}
